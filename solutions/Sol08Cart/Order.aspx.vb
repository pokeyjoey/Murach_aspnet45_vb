...
Partial Class Order
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        'bind dropdown amd update page hit count on first load;                 
        If Not IsPostBack Then
            ddlProducts.DataBind()

            'get hit count from cache or set to 1
            Dim hitCount As Integer = CInt(Cache.Get("HitCount"))
            hitCount += 1
            Cache.Insert("HitCount", hitCount, Nothing, DateTime.Now.AddMinutes(10), Cache.NoSlidingExpiration)
        ...
        If Session("FirstName") IsNot Nothing Then
            Dim firstName As String = Session("FirstName").ToString
            lblWelcome.Text = "Welcome back, " & firstName & "!"
        End If

        'get last update from cache, or set to now and reload cache, then display
        Dim lastUpdateTime As Object = Cache.Get("LastUpdateTime")
        If lastUpdateTime Is Nothing Then
            lastUpdateTime = DateTime.Now
            Cache.Insert("LastUpdateTime", lastUpdateTime, Nothing, DateTime.Now.AddMinutes(10), Cache.NoSlidingExpiration)
        End If
